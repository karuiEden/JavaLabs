...
где $\text{bias} = 2^{k-1} - 1 $, где $k$ - число бит экспоненты, следовательно, для
double $\text{bias} = 2^{10}-1=1023$,
...
имея диапазон $[0, 2047] $, но $0$ используется для Subnormal numbers, а $2047$ — для бесконечности или NaN.
Используется для
