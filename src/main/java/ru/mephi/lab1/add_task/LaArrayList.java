...
/**
 * A resizable list of strings backed by an array. Elements are stored in insertion order and
 * indices are zero-based. Null elements are permitted. This class is not thread-safe. Capacity
 * grows by about 1.5x when needed; a zero-capacity list grows to {@value #DEFAULT_CAPACITY} on
 * first add.
 */
public class LaArrayList implements ILaList {
  ...
  /**
   * Creates a list with the default initial capacity ({@value #DEFAULT_CAPACITY}). The initial size
   * is 0.
   */
  public LaArrayList() {
